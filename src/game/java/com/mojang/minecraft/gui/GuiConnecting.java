...
public class GuiConnecting extends GuiScreen {
		...
		if (timer > 1) {
			if (this.minecraft.networkManager.netHandler.webSocket == null) {
				...
				if (this.minecraft.networkManager.netHandler.webSocket == null) {
					...
					this.minecraft.setCurrentScreen(new ErrorScreen("Failed to connect",
				...
				if (this.minecraft.networkManager.netHandler.webSocket
						.getState() == EnumEaglerConnectionState.CONNECTED) {
					if (!this.minecraft.networkManager.successful) {
						this.minecraft.networkManager.netHandler.send(PacketType.IDENTIFICATION,
								new Object[] { Byte.valueOf((byte) 7), username, key, Integer.valueOf(0) });
						this.minecraft.networkManager.successful = true;
						this.connected = true;
					}
				} else if (this.minecraft.networkManager.netHandler.webSocket
	...
	public void render(int i, int i2) {
		...
		if ((var2 = EagRuntime.steadyTimeMillis()) - this.start < 0L || var2 - this.start >= 20L) {
			...
			var6.pos((float) var4, (float) var5, 0.0F).tex((float) var4 / var10, (float) var5 / var10).color(4210752)
					.endVertex();
