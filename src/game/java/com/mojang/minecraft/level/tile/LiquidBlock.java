...
public class LiquidBlock extends Block {
	...
	public void update(Level level, int x, int y, int z, Random rand) {
		...
		if (this.type == LiquidType.WATER || !var8) {
			var8 = var8 | this.flow(level, x - 1, y, z) | this.flow(level, x + 1, y, z) | this.flow(level, x, y, z - 1)
					| this.flow(level, x, y, z + 1);
		}
	...
	public final boolean canRenderSide(Level level, int x, int y, int z, int side) {
		int var6;
		return x >= 0 && y >= 0 && z >= 0 && x < level.width && z < level.height
				? ((var6 = level.getTile(x, y, z)) != this.movingId && var6 != this.stillId
