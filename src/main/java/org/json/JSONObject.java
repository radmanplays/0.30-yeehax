	...
	public JSONObject(JSONTokener x) throws JSONException {
		...
		for (;;) {
			...
			if (key != null) {
				// Check if key exists
				/*
				 * if (this.opt(key) != null) { // key already exists throw
				 * x.syntaxError("Duplicate key \"" + key + "\""); }
				 */
...
// Shave off trailing zeros and decimal point, if possible.
	...
	@Nullable
	public Object getOptional(String key) throws JSONException {
		if (key == null) {
			return null;
		}
		Object object = this.opt(key);
	 ...
	 * double, the {@link BigDecimal#BigDecimal(double)} constructor will be used.
	 * See notes on the constructor for conversion issues that may arise.
	 *
	 * @param key A key string.
	 * @return The numeric value.
	 * @throws JSONException if the key is not found or if the value cannot be
	...
	/**
	 * Removes all of the elements from this JSONObject. The JSONObject will be
	 ...
	 *                     {@link Float} values will be converted exactly. When
	 *                     <code>false</code>, they will be converted to
	...
	/**
	 * Get an optional JSONObject associated with a key, or the default if there is
	 * no such key or if the value is not a JSONObject.
	 *
	 * @param key          A key string.
	 * @param defaultValue The default.
	 ...
	 * to have a finite value. If either value is not finite (NaN or
	 * &#177;infinity), then this function will always return false. If both numbers
	 * are finite, they are first checked to be the same type and implement
	 * {@link Comparable}. If they do, then the actual
	 * {@link Comparable#compareTo(Object)} is called. If they are not the same
	 * type, or don't implement Comparable, then they are converted to
	 * {@link BigDecimal}s. Finally the BigDecimal values are compared using
